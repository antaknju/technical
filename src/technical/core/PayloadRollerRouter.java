...
public class PayloadRollerRouter extends PayloadRollerConveyor
{
    public boolean invert = false;

    public Effect landingEffect = TFx.edgeCloud;

    public TextureRegion baseRegion;

    public PayloadRollerRouter(String name){
        super(name);
    ...
    protected TextureRegion[] icons() {
        return new TextureRegion[]{baseRegion, rollerRegions[0]};
    }
    ...
    public class PayloadRouterBuild extends PayloadConveyorBuild{
        ...
        public void updateTile()
        {
            ...
            if(Angles.angleDist(smoothRot, rotdeg()) < thresh && Angles.angleDist(lastSmoothRot, rotdeg()) >= thresh)
            {
                spawnEdgeParticles();
            }
        }
        ...
        {
            float length = size * tilesize;

            float angle = smoothRot;

            float px = Angles.trnsx(angle + 90f, length / 2f);
            float py = Angles.trnsy(angle + 90f, length / 2f);

            // top & bottom edge
            landingEffect.at(x + px, y + py, angle, length);
            landingEffect.at(x - px, y - py, angle, length);
        }
        ...
        public void draw()
        {
            ...
            Draw.rect(currentRollerRegion(), x, y, smoothRot);

            Draw.z(Layer.block - 0.3f);

            Draw.rect(capRegion, x, y, smoothRot);
            Draw.rect(capRegion, x, y, smoothRot + 180f);
        ...
        public void buildConfiguration(Table table){
            ItemSelection.buildTable(PayloadRollerRouter.this, table,
                    content.blocks().select(PayloadRollerRouter.this::canSort).<UnlockableContent>as()
                            .add(content.units().select(PayloadRollerRouter.this::canSort).as()),
