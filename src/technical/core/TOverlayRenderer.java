...
public class TOverlayRenderer extends OverlayRenderer
{
    ...
    private void updateCoreEdges(){
        ...
        Seq<CoreBlock.CoreBuild> teams = new Seq<>();
    ...
    public void drawTop()
    {
        ...
        if (Core.input.keyDown(Binding.control) && playerUnit != null)
        {
            Draw.z(Layer.overlayUI);
            Drawf.dashCircle(
                    playerUnit.x, playerUnit.y, TVars.ControlRange,
                    select == null ? Pal.accent : TVars.isInPlayerRange(select) ? Pal.accent : Pal.remove
            );
        }
        ...
        if(select != null && (!(select instanceof Unitc u) || u.isAI())){
            Draw.mixcol(TVars.isInPlayerRange(select) ? Pal.accent : Pal.remove, 1f);
            Draw.alpha(unitFade);
    ...
    public void checkApplySelection(Unit u){
        if(unitFade > 0.001f && lastSelect == u){
            ...
            Draw.mixcol(prev.a > 0.001f ? prev.lerp(TVars.isInPlayerRange(u) ? Pal.accent : Pal.remove, unitFade) : TVars.isInPlayerRange(u) ? Pal.accent : Pal.remove, Math.max(unitFade, prev.a));
        }
    }
