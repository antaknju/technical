...
public class TPayloadBlock extends KineticBlock
{
    public float payloadSpeed = 0.7f, payloadRotateSpeed = 5f;

    public TextureRegion topRegion, outRegion, inRegion;

    public boolean blendsInput = true;
    public boolean blendsOutput = true;

    public TPayloadBlock(String name)
    {
        super(name);
    ...
    public void load(){
        ...
        outRegion = findPayloadRegion("-out");
        inRegion = findPayloadRegion("-in");
    }
    ...
    protected TextureRegion findPayloadRegion(String suf)
    {
        TextureRegion region = Core.atlas.find(name + suf);

        if(!region.found()) region = Core.atlas.find(Technical.name + "-payload" + suf + "-" + size);

        return region;
    }

    public static boolean blends(Building build, int direction, boolean blendsInput, boolean blendsOutput) 
    {
        // Calculate which absolute directions represent the front and back of the building
        boolean isFront = direction == build.rotation;
        boolean isBack = direction == (build.rotation + 2) % 4;

        // If it does not blend outputs, force a connection to the front even with no adjacent tiles
        if (!blendsOutput && isFront) return true;

        // If it does not blend inputs, force a connection to the back even with no adjacent tiles
        if (!blendsInput && isBack) return true;

        // If it does not blend inputs, it should strictly reject side connections (anything that isn't front or back)
        if (!blendsInput && !isFront && !isBack) return false;

        // --- Original Blending Logic for adjacent blocks ---
    ...
    public class TPayloadBuild<P extends Payload> extends KineticBuild
    {
        ...
        {
            return TPayloadBlock.blends(this, direction, blendsInput, blendsOutput);
        }
