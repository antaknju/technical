...
public class ThermalPump extends ThermalLiquidBlock 
{
    ...
    public void setBars(){
        super.setBars();
        removeBar("liquid");
        addLiquidBar((ThermalPumpBuild b) -> b.liquidDrop);
    }

    protected boolean canPump(Tile tile){
        return tile != null && T.getTLiquidDrop(tile) != null;
    }
    ...
    public boolean canPlaceOn(Tile tile, Team team, int rotation){
        if(isMultiblock()){
            ...
            for(Tile other : tile.getLinkedTilesAs(this, tempTiles)){
                if(T.getTLiquidDrop(other) == null) continue;
                if(T.getTLiquidDrop(other) != last && last != null) return false;
                last = T.getTLiquidDrop(other);
            }
            return last != null;
        }else{
            return canPump(tile);
    ...
    public void drawPlace(int x, int y, int rotation, boolean valid)
    {
        ...
        if(valid && tile != null){
            ...
            for(Tile other : tile.getLinkedTilesAs(this, tempTiles)){
                if(canPump(other)){
                    if(liquidDrop != null && T.getTLiquidDrop(other) != liquidDrop){
                        liquidDrop = null;
                        break;
                    }
                    liquidDrop = T.getTLiquidDrop(other);
    ...
    public class ThermalPumpBuild extends ThermalLiquidBuild 
    {
        public float warmup, totalProgress;
        public float consTimer;
        public float amount = 0f;
        public TLiquid liquidDrop = null;
