...
public class ExtendableCrafter extends Extendable
{
    ...
    public ExtendableCrafter(String name) {
        super(name);
        update = true;
        solid = true;
        hasItems = true;
    ...
    public class ExtendableCrafterBuild extends ExtendableBuild 
    {
        ...
        public void updateTile()
        {
            ...
            if(efficiency > 0 && hasRequiredExtensions())
            {
        ...
        {
            float eff = super.efficiencyScale();
            ...
            if (!ignoreLiquidFullness)
            {
                ...
                eff *= (dumpExtraLiquid ? Math.min(max, 1f) : scaling);
            }

            eff = Mathf.clamp(eff, 0f, maxEfficiency);

            return eff;
        }

        public void craft(){
            if (!Mathf.chance(getTotalStat(TechStat.materialSaveChance))) consume();

            int multiplier = 1 + (Mathf.chance(getTotalStat(TechStat.doubleProductionChance)) ? 1 : 0);

            if(outputItems() != null){
                for(var output : outputItems()){
                    for(int i = 0; i < output.amount * multiplier; i++){
                        offload(output.item);
                    }
                }
            }
