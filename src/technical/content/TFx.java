...
public class TFx {
    ...
    bend = new Effect(30, e -> {
        randLenVectors(e.id, 12, 4f + e.fin() * 5f, (x, y) -> {
            color(TCol.copper);
            Fill.square(e.x + x, e.y + y, 0.2f + e.fout() * 1f, 40 + e.fin() * 10f);
        });
    }),
    ...
    roll = new Effect(40, e -> {
        randLenVectors(e.id, 10, 1f + e.fin() * 4f, (x, y) -> {
            ...
            Fill.square(e.x + x, e.y + y, 0.5f + e.fout() * 1f, 40 + e.fin() * 20f);
        });
    }),
    ...
    // sulphur_front_spark = new Effect(12f, e -> {
    //     color(Color.white, TCol.sulphur_light, e.fin());
    ...
    //     randLenVectors(e.id, 5, 2f + e.fin() * 20f, (x, y) -> {
    //         Fill.circle(e.x + x, e.y + y, 0.2f + e.fout() * 3f);
    //     });

    //     color();
    // }),

    // sulphured = new Effect(120f, e -> {
    //     color(TCol.sulphur);
    //     alpha(Mathf.clamp(e.fin() * 2f));

    //     Fill.circle(e.x, e.y, e.fout() * 2f);
    // }),

//     sandstorm_trail = new Effect(10.0F, (e) -> {
//         // disperse
//         Draw.color(TCol.sand, e.color, e.fin());
//         Lines.stroke(0.5F + e.fout() * 1.3F);
//         rand.setSeed((long)e.id);

//         for(int i = 0; i < 2; ++i) {
//             float rot = e.rotation + rand.range(15.0F) + 180.0F;
//             v.trns(rot, rand.random(e.fin() * 27.0F));
//             Lines.lineAngle(e.x + v.x, e.y + v.y, rot, e.fout() * rand.random(2.0F, 7.0F) + 1.5F);
//         }

//         // missile trail smoke
//         Color c = Color.grays(0.6f).lerp(TCol.sand, 0.5f).a(0.1f);
//         float intensity = 1F;
//         Draw.color(c, 0.3F);

//         for(int i = 0; i < 3; ++i) {
//             final int fi = i;
//             rand.setSeed((long)(e.id * 2 + fi));
//             float lenScl = rand.random(0.3F, 0.7F);
//             e.scaled(e.lifetime * lenScl, (e2) -> {
//                 Angles.randLenVectors((long)(e2.id + fi - 1), e2.fin(Interp.pow10Out), (int)(2.9F * intensity), 13.0F * intensity, (x, y, in, out) -> {
//                 float fout = e2.fout(Interp.pow5Out) * rand.random(0.5F, 1.0F);
//                 float rad = fout * (2.0F + intensity) * 2.35F;
//                 Fill.circle(e2.x + x, e2.y + y, rad);
// //                 Drawf.light(e2.x + x, e2.y + y, rad * 2.5F, c, 0.5F);
// //                 });
// //             });
// //         }
// //    }),

//     hit_electric = new Effect(40.0F, (e) -> {
//         Draw.color(TCol.electricity);
//         Lines.stroke(e.fout() * 1.6F);
//         Angles.randLenVectors((long)e.id, 18, e.finpow() * 12.0F, e.rotation, 360.0F, (x, y) -> {
//             float ang = Mathf.angle(x, y);
//             Lines.lineAngle(e.x + x, e.y + y, ang, e.fout() * 3.0F + 1.0F);
//         });
//     }),

//     // sulphurSplat = new Effect(400f, 300f, b -> {
//     //     float intensity = 3f;

//     //     color(AourusItems.sulphur_fluid.color);
//     //     for(int i = 0; i < 4; i++){
//     //         rand.setSeed(b.id*2 + i);
...
//     impact_wave = new Effect(60f, e -> {
//         color(e.color);

//         stroke(e.fout() * 1.5f);

//         randLenVectors(e.id, 24, 8f + e.finpow() * e.rotation, (x, y) -> {
//             lineAngle(e.x + x, e.y + y, Mathf.angle(x, y), e.fout() * 6 + 1f);
//         });

//         e.scaled(30f, b -> {
//             Lines.stroke(5f * b.fout());
//             Lines.circle(e.x, e.y, b.finpow() * 40f);
//         });
//     }),


    // // massive_missile_explosion = new Effect(30, e -> {
    ...
    // //     Drawf.light(e.x, e.y, 50f, TCol.sulphur, 0.8f * e.fout());
    ...
    // // massive_shoot = new Effect(8, e -> {
    // //     color(TCol.sulphur_light, TCol.sulphur, e.fin());
    // //     float w = 1.2f + 7 * e.fout();
    // //     Drawf.tri(e.x, e.y, w, 25f * e.fout(), e.rotation);
    // //     Drawf.tri(e.x, e.y, w, 4f * e.fout(), e.rotation + 180f);
    // // }),

    // ultra_vent_steam = new Effect(140f, e -> {
    //     color(e.color, Color.valueOf("657bbd"), e.fin());

    //     alpha(e.fslope() * 0.78f);

    //     float length = 3f + e.finpow() * 15f;
    //     rand.setSeed(e.id);
    //     for(int i = 0; i < rand.random(5, 6); i++){
    //         v.trns(rand.random(360f), rand.random(length));
    //         Fill.circle(e.x + v.x, e.y + v.y, rand.random(1f, 3f) + e.fslope() * 1.1f);
    //     }
    // }).layer(Layer.darkness - 1),

    // small_impact_wave = new Effect(60f, e -> {
    //     color(e.color);

    //     stroke(e.fout() * 1f);

    //     randLenVectors(e.id, 16, 8f + e.finpow() * e.rotation, (x, y) -> {
    //         lineAngle(e.x + x, e.y + y, Mathf.angle(x, y), e.fout() * 3 + 1f);
    //     });

    //     e.scaled(30f, b -> {
    //         Lines.stroke(3f * b.fout());
    //         Lines.circle(e.x, e.y, b.finpow() * 20f);
    //     });
    // })

    // // shoot_megalton = new Effect(8, e -> {
    // //     color(TCol.graphite_light, TCol.graphite_dark, e.fin());
    // //     float w = 1f + 5 * e.fout();
